...
//Подсчет кол-ва соседей (просто берем соседние клетки)
//Добавляем n и берем по модулю, чтобы не считать соседей с минусовых координат
{
    int k = 0;
...
//Функция, которая переводит матрицу на след итерацию
{
    int matr[n][m]; //дополнительная матрица для подсчета новых клеток

    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < m; ++j)
        {
            int k = NumberOfNeighbors(generation, n, m, i, j); //Функцция, которая описана выше
            matr[i][j] = generation[i][j];
            if (k == 3) matr[i][j] = 1;             // проверяем статус клетки
            if (k != 2 && k != 3) matr[i][j] = 0;   // живая/неживая
        }
    }
...
void toImage (int **generation, int n, int m, char* output, char header[54]) //Функция для перевода матрицы в картинку
{
    FILE *outputfile;
    outputfile = fopen (output, "w");
    for (int i = 0; i < 54; ++i)
        fprintf(outputfile, "%c", header[i]); //header у всех используемых файлов одинаковый, поэтому просто его копируем в новый файл
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < m; ++j)
        {
            if (generation[i][j] == 0)
                fprintf(outputfile, "%c%c%c", -1, -1, -1); // Если 0 значит клетка не живая, заносим (-1, -1, -1) в битах (255,255,255) т.е белый цвет 
            if (generation[i][j] == 1)
                fprintf(outputfile, "%c%c%c", 0, 0, 0); // Заносим черный цвет
